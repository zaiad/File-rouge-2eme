...
function Product() {
  ...
  const addproduct = async (e) => {
    ...
    const product_add = await axios.post(
      `http://localhost:8080/manager/add-product`,
      formData
    );
    if (product_add.data.message) {
      toast.success(product_add.data.message);
      setShowModal(false);
      // setTimeout(() => { window.location.reload(false) }, "1000")
    } else toast.warning(product_add.data.message);
  };
  const editproduct = async (e) => {
    e.preventDefault();
    const product_edit = await axios.put(
      `http://localhost:8080/manager/updat-product/${edit_product._id}`,
      edit_product
    );
    if (product_edit.data.message) {
      toast.success(product_edit.data.message);
      setShowModalEdit(false);
    } else toast.warning(product_edit.data.message);
  };
  const deletproduct = async (id, status, e) => {
    e.preventDefault();
    const delete_product = await axios.delete(
      `http://localhost:8080/manager/delet-product/${id}/${status}`
    );
    if (delete_product.data.message) {
      toast.success(delete_product.data.message);
    } else toast.warning(delete_product.data.message);
  };
