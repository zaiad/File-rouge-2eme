...
const addCommand = async (req, res) => {
  const { body } = req;
  if (!body.client || !body.produit || !body.quantite)
    return res
      .status(400)
      .json({ message: "Fill the all fields to add command" });
  const find_client = await User.findOne({ _id: body.client });
  const find_produit = await Produit.findOne({ _id: body.produit });
  return res.send(find_produit);
  //   const find_status_demende = await Status.findOne({ name: "demandé" });
  //   if (!find_client) return res.status(400).json({ message: "Client not find" });
  //   if (!find_produit)
  //     return res.status(400).json({ message: "Produit not find" });
...
const assignCommand = async (req, res) => {
  ...
  const update_command = await Command.findByIdAndUpdate(
    { _id: command },
    { livreur: livreur }
  );
  res.json({
    message: `Commend ${command} asined to livreur ${find_livreur.username}`,
  });
};
...
const statusCommand = async (req, res) => {
  ...
  const status_command = await Command.findByIdAndUpdate(
    { _id: c },
    { status: s }
  );
